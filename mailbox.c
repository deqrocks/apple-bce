...
{
    /*
     * Windows vtable +0x100 pauses the BCE mailbox channel before the power
     * callback. Keep the channel mutex held across that phase.
     */
...
int bce_mailbox_send_locked_timeout(struct bce_mailbox *mb, u64 msg, u64* recv, unsigned int timeout_ms)
{
    ...
    if (atomic_cmpxchg(&mb->mb_status, 0, 1) != 0) {
        status = -EEXIST; // We don't support two messages at once
        goto out;
    }
...
int bce_mailbox_send_timeout(struct bce_mailbox *mb, u64 msg, u64* recv, unsigned int timeout_ms)
{
    ...
    /*
     * Windows brackets every mailbox transaction with BCE channel lock/unlock
     * (+0x9c8/+0x9d0), including isolated sends.
     */
    mutex_lock(&mb->channel_lock);
...
int bce_mailbox_send_no_reply(struct bce_mailbox *mb, u64 msg)
{
    ...
    /* Fire-and-forget mailbox sends still use the same BCE channel lock in Windows. */
    mutex_lock(&mb->channel_lock);
