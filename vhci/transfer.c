...
static bool bce_vhci_transfer_queue_defer_event(struct bce_vhci_transfer_queue *q, struct bce_vhci_message *msg)
{
    ...
    if (!lm) {
        pr_warn("bce-vhci: [%02x] failed to defer transfer queue event cmd=%x status=%x\n",
...
static void bce_vhci_transfer_queue_remove_pending(struct bce_vhci_transfer_queue *q)
{
    ...
    if (bce_vhci_transfer_queue_is_ep0(q) && !list_empty(&q->evq))
        pr_debug("bce-vhci: EP0 event remove-pending dev=%u port=%d evq=%zu active=%u paused_by=%x stalled=%u qstate=%u remaining=%u\n",
...
int bce_vhci_transfer_queue_do_resume(struct bce_vhci_transfer_queue *q)
{
    ...
    u8 endp_addr = (u8) (q->endp->desc.bEndpointAddress & 0x8F);
    if (bce_vhci_transfer_queue_is_ep0(q))
        pr_debug("bce-vhci: tq resume begin dev=%u port=%d ep=%02x active=%u paused_by=%x stalled=%u state=%u remaining=%u\n",
    ...
    spin_unlock_irqrestore(&q->urb_lock, flags);
    if (bce_vhci_transfer_queue_is_ep0(q))
        pr_debug("bce-vhci: tq resume done dev=%u port=%d ep=%02x active=%u paused_by=%x stalled=%u state=%u remaining=%u\n",
                q->dev_addr, bce_vhci_transfer_queue_port(q), q->endp_addr,
                q->active, q->paused_by, q->stalled, q->state,
                q->remaining_active_requests);
    return 0;
...
int bce_vhci_transfer_queue_pause(struct bce_vhci_transfer_queue *q, enum bce_vhci_pause_source src)
{
    ...
    if (bce_vhci_transfer_queue_is_ep0(q) || ret)
        pr_debug("bce-vhci: tq pause dev=%u port=%d ep=%02x src=%x ret=%d paused_by=%x->%x active=%u stalled=%u state=%u\n",
                q->dev_addr, bce_vhci_transfer_queue_port(q), q->endp_addr,
...
static void bce_vhci_transfer_queue_reset_w(struct work_struct *work)
{
    ...
    mutex_lock(&q->pause_lock);
    spin_lock_irqsave(&q->urb_lock, flags);
    pr_debug("bce-vhci: tq reset start dev=%u port=%d ep=%02x active=%u paused_by=%x stalled=%u state=%u remaining=%u\n",
            q->dev_addr, bce_vhci_transfer_queue_port(q), q->endp_addr,
            q->active, q->paused_by, q->stalled, q->state,
            q->remaining_active_requests);
    if (!q->stalled) {
        spin_unlock_irqrestore(&q->urb_lock, flags);
        mutex_unlock(&q->pause_lock);
        pr_debug("bce-vhci: tq reset skipped dev=%u port=%d ep=%02x stalled=0 active=%u paused_by=%x state=%u\n",
                q->dev_addr, bce_vhci_transfer_queue_port(q), q->endp_addr,
    ...
    if (q->sq_out)
        bce_cmd_flush_memory_queue(q->vhci->dev->cmd_cmdq, (u16) q->sq_out->qid);
    pr_debug("bce-vhci: tq reset command dev=%u port=%d ep=%02x\n",
            q->dev_addr, bce_vhci_transfer_queue_port(q), q->endp_addr);
    ...
    pr_debug("bce-vhci: tq reset done dev=%u port=%d ep=%02x active=%u paused_by=%x stalled=%u state=%u remaining=%u\n",
            q->dev_addr, bce_vhci_transfer_queue_port(q), q->endp_addr,
            q->active, q->paused_by, q->stalled, q->state,
            q->remaining_active_requests);
}
...
static void bce_vhci_transfer_queue_resume_w(struct work_struct *work)
{
    ...
    pr_debug("bce-vhci: tq async resume start dev=%u port=%d ep=%02x active=%u paused_by=%x stalled=%u state=%u remaining=%u\n",
            q->dev_addr, bce_vhci_transfer_queue_port(q), q->endp_addr,
            q->active, q->paused_by, q->stalled, q->state,
            q->remaining_active_requests);
    ret = bce_vhci_transfer_queue_resume(q, BCE_VHCI_PAUSE_INTERNAL_WQ);
    pr_debug("bce-vhci: tq async resume done dev=%u port=%d ep=%02x ret=%d active=%u paused_by=%x stalled=%u state=%u remaining=%u\n",
            q->dev_addr, bce_vhci_transfer_queue_port(q), q->endp_addr,
...
int bce_vhci_urb_create(struct bce_vhci_transfer_queue *q, struct urb *urb)
{
    ...
    if (is_control && q->endp_addr == 0x00 &&
        !q->active && !q->stalled &&
        q->paused_by == BCE_VHCI_PAUSE_INTERNAL_WQ) {
        pr_warn("bce-vhci: EP0 enqueue scheduling async stale internal pause recovery dev=%u port=%d state=%u remaining=%u urb_reject=%d\n",
    ...
    if (vurb->is_control && (!q->active || q->stalled || q->paused_by))
        pr_debug("bce-vhci: EP0 enqueue dev=%u port=%d active=%u paused_by=%x stalled=%u state=%u remaining=%u urb_reject=%d\n",
                q->dev_addr, bce_vhci_transfer_queue_port(q),
                q->active, q->paused_by, q->stalled, q->state,
                q->remaining_active_requests, atomic_read(&urb->reject));
